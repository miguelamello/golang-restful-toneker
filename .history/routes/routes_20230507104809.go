...
func GetToken(res http.ResponseWriter, req *http.Request) {
    ...
    if !email {
        response := validation.FormatResponse400("400", "email not valid. Verify email presence in header variables or request a new one. See Reference documentation.")
        fmt.Fprint(res, jsonString)
        return
    }
