...
func GetToken(res http.ResponseWriter, req *http.Request) {
    ...
    // Validate the bearer string from the header 
    valid := validation.ValidateBearerString(req.Header.Get("bearerToken"))
    if !valid {
        fmt.Fprintf(res, "{ status: 400, message: 'bearerToken not valid. Verify bearerToken presence in header variables or request a new one. See Reference documentation.' }")
        return
    }
